...
class TestNewsletterSubscribeResource:
    def test_post_new_user_subscribe_explicit_active(self, api_client):
        data = NEWSLETTER_SUBSCRIBE_DATA.copy()
        r = api_client.post(url_for('api.newsletter_subscribes_resource'), data=data)
        assert r.status_code == 201

    def test_post_new_user_subscribe_implicit_active(self, api_client):
        data = NEWSLETTER_SUBSCRIBE_DATA.copy()
        data['isActive'] = None
        r = api_client.post(url_for('api.newsletter_subscribes_resource'), data=data)
        assert r.status_code == 201

    def test_patch_modify_user_active(self, api_client, newslettersubscribe):
